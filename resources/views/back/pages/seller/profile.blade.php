                    ...
                    <div class="col-sm order-2 order-sm-1">
                        <div class="d-flex align-items-center mt-3 mt-sm-0">
                            <div class="flex-shrink-0 pic-avator">
                                <a href="javascript:;" class="avator-change" onclick="event.preventDefault();document.getElementById('sellerProfilePicturFile').click();"><i class="mdi mdi-pencil-outline" style="font-size: 20px;"></i></a>
                                <div class="avatar-xl me-3">
                                    <img src="{{ $seller->picture }}" alt="" class="img-fluid rounded-circle d-block" id="sellerProfilePicture">
                                </div>
                                <input type="file" name="sellerProfilePicturFile" id="sellerProfilePicturFile" class="d-none" style="opacity: 0;">
                            </div>
                            <div class="flex-grow-1">
                                <div>
                                    <h5 class="font-size-16 mb-1" id="sellerProfileName">{{ $seller->name }}</h5>
                                    <div class="d-flex flex-wrap align-items-start gap-2 gap-lg-3 text-muted font-size-13">
                                        <div id="sellerProfileEmail"><i class="mdi mdi-circle-medium me-1 text-success align-middle"></i>{{ $seller->email }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    window.addEventListener('updateSellerInfo', function() {
        $('#sellerProfileName').html(event.detail[0].sellerName);
        $('#sellerProfileEmail').html(event.detail[0].sellerEmail);
    });

    $('input[type="file"][name="sellerProfilePicturFile"][id="sellerProfilePicturFile"]').ijaboCropTool({
